...
function lianDong(newValue,oldValue) {
    if (newValue!=oldValue) {  
        $('#deployCode').combobox({  
            url: "ticket/findByAttrBizTicketDeployAndApply.zb?depolyState=1&groupName=<%=groupName%>&auditingState=3&fromOrg=" + newValue  
        });  
    }  
		...
		<label for="deployCode">调配单号:</label>   
		 ...
		 <td>
			<input id="deployCode"  name="deployCode" class="easyui-combobox" 
								style="width: 150px;"
								data-options="
							url:'ticket/findByAttrBizTicketDeployAndApply.zb?depolyState=1&groupName=<%=groupName%>&auditingState=3&fromOrg=<%=sys_orgid %>', 
							required:true,
							prompt:'*必填*',
