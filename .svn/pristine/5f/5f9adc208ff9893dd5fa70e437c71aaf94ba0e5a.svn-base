package com.solidextend.core.web.converter;
...
public class StringToDateConverter implements Converter<String, Date> {
	...
	public Date convert(String source) {
		...
		if (StringUtils.isNotEmpty(dateFormatString)) {
			if(source.length()<dateFormatString.length()){
				dateStyle = dateFormatString.substring(0,source.length());
			}else{
		...
		try {
			...
			throw new IllegalArgumentException(String.format("类型转换失败，需要格式"+ dateStyle + "，但格式是[%s]", source));
		}
	}
}
