...
function doSearch(){
	...
	$.post("ticket/findByAttrBizTicketOutDetail.zb",formdata,function(data){
			if(data.success){
...
function saveData(){
	...
	$.post("ticket/saveBizTicketOutDetail.zb",formdata,function(data){
			if(data.success){
...
function deleteRows(){
	...
	if(rows.length<1){
		...
		var obj={};
        		...
        		var outDetailId=new Array();
				for(var i=0;i<rows.length;i++){
					outDetailId.push(rows[i].outDetailId);
				}
				obj.outDetailId=outDetailId;
		
		
		
		$.post("ticket/deleteBizTicketOutDetail.zb",obj,function(data){
			if(data.success){
...
<div id="dataTabelToolBar" style="height: auto">
			...
			<form id="queryForm">
					...
					:<input name="outDetailId" class="easyui-textbox" style="width:100px"> 
    ...
    <thead>   
        <tr>
      			<th data-options="field:'outDetailId',width:100,checkbox:true" ></th>
      			<th data-options="field:'outId',width:100" ></th>  
...
<div id="dataDialog" class="easyui-dialog" title="信息" style="width:400px;height:200px;padding: 15px"   
    ...
    <table>
    	<input type="hidden" name="outDetailId" value=""/>
