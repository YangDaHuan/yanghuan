...
<%@ page
	import="com.solidextend.sys.security.UserDetails,org.apache.shiro.SecurityUtils,java.util.List,com.solidextend.sys.role.vo.Role"%>
<%@ page import="com.solidextend.sys.security.UserDetails,org.apache.shiro.SecurityUtils,java.util.List"%>
<html>
<head>
...
<jsp:include page="../jsp/ticket/combo.jsp" />
...
<body topmargin=0 leftmargin=0 rightmargin=0 bottomMargin=0
	...
	for (Role role : roles) {
		sys_roleids=sys_roleids+role.getId()+",";
	}
	sys_roleids=sys_roleids.substring(0,sys_roleids.length()-1);
	param=param+"sys_userid="+sys_userid+";sys_username="+sys_username+";sys_orgid="+sys_orgid+";sys_orgcode="+sys_orgcode+";sys_roleids="+sys_roleids+";";
	 */
%>
	<div id=mengban
		...
		<table width=100% height=100%>
			<tr>
				<td width=100% style="text-align: center; vertical-align: middle"><img
					src="../raqsoft/images/loading.gif"><br>正在加载报表......</td>
			</tr>
		</table>
	</div>
				...
				<div data-options="region:'north',border:false">
					<center>
						...
						<table id="param_tbl" align=center>
							<tr>
						<!-- <td>
			<%-- sys_userid:<%= sys_userid%><br>
sys_username:<%= sys_username%><br>
sys_orgid:<%= sys_orgid%><br>
sys_roleids:<%= sys_roleids%><br> --%>
			</td> --> 
								<td><report:param name="form1"
										paramFileName="<%=paramFile%>" needSubmit="no"
										params="<%=param.toString()%>"
										hiddenParams="<%=param.toString()%>" needImportEasyui="no"
										resultContainer="reportContainer" resultPage="<%=resultPage%>" />
								</td>
				...
				<div id=reportContainer data-options="region:'center',border:false"
					...
					<report:html name="report1" reportFileName="<%=report%>"
						funcBarLocation="no" needScroll="<%=scroll%>"
						generateParamForm="no" params="<%=param.toString()%>"
						exceptionPage="/reportJsp/myError2.jsp"
						appletJarName="/raqsoftReportApplet.jar" scrollWidth="100%"
						scrollHeight="100%" needImportEasyui="no" />
				</div>
			</div>
		</div>
