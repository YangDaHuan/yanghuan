...
$.post("ticket/saveBizTicketApply.zb",{jsonStr:jsonStr},function(data){
			if(data.success&&data.msg!=""){
				$.messager.alert("信息","提交成功!<br>您的申请单号为:"+data.msg ,"info",function(){
					location=location;
	　　　　　　});
				addRow();
				$("#dataForm").form("clear");
...
<body class="easyui-layout" data-options="border:false">
	<div data-options="region:'center',title:'票卡申领'"
		style="padding: 20px; background: #eee;">
		<form id="dataForm">

			<table>
				<input type="hidden" name="applyId" value="" />
				<input name="applyType" type="hidden" value="1">
				<input name="auditingState" type="hidden" value="1">

				<caption align="top" style="font-size: 24px; color: #006600;">票卡申领单</caption>
				<tr>
					<td>&nbsp</td>
				</tr>
				<!-- <tr><td><label for="applyCode">申请单号:</label>   </td> 
		<td>
			<input name="applyCode1" class="easyui-textbox" style="width:150px" data-options="required:false,readonly:true,validType:['length[3,30]']"> 
       </td>
         </tr> -->
				<tr>
					<td><label for="applyOrg">申领部门:</label></td>
					<td><input id="applyOrg" class="easyui-combotree"
						name="applyOrg" style="width: 150px;"
						data-options="
								value:'<%=sys_orgid%>',
					...
					<td><label for="toOrg">目标部门:</label></td>
					<td><input id="toOrg" class="easyui-combotree" name="toOrg"
						style="width: 150px;"
						data-options="
				...
				<tr>
					<td><label for="applyContacts">申请人:</label></td>
					<td><input name="applyContacts" class="easyui-textbox"
						style="width: 150px" value="<%=sys_username%>" readonly>
					</td>
					<td><label for="applyPhone">申请人电话:</label></td>
					<td><input name="applyPhone" class="easyui-textbox"
						style="width: 150px"></td>
				</tr>
				<tr>
					<td><label for="description">备注:</label></td>
					<td colspan=3><input name="description" class="easyui-textbox"
						style="width: 403px; height: 50px"></td>
				</tr>
		...
		">删除</a>
						...
						<table id="dataTable" class="easyui-datagrid"
							data-options="toolbar:'#dataTabelToolBar',rownumbers:true">
							<thead>
								<tr>
									<!--<th data-options="field:'applyDetailId',width:100,checkbox:true" ></th> -->
									<th
										data-options="field:'ticketTypeId',width:114,formatter:typeFormatter,editor:{type:'combobox',options:{data:getTicketTypes(),valueField:'ticketTypeId',textField:'ticketTypeName',editable:false,required:true,validType:'leixing'}}">票卡类型</th>
									<th
										data-options="field:'ticketStateId',width:105,formatter:stateFormatter,editor:{type:'combobox',options:{data:getTicketStates(),valueField:'ticketStateId',textField:'ticketStateName',
																			editable:false,
																			required:true
									...
									<th
										data-options="field:'ticketMoney',width:105,editor: { type: 'numberbox', options: { min:0,max:1000000,required: false} }">金额</th>
									<th
										data-options="field:'ticketNumber',width:105,editor: { type: 'numberbox', options: { min:0,max:1000000,required: true,validType:'jiaoyan'} }">数量(张/本)</th>
				...
				<tr>
					<td colspan=4 align="center"><a class="easyui-linkbutton"
						data-options="onClick:submitApply,iconCls:'icon-ok'">提交</a></td>
				</tr>
			</table>
		</form>
</body>
