...
(function($, window, document, undefined) {
	...
	Zmenu.prototype.setStyles = function(ele) {
		...
		if (this.active) {
			this.active.css('background-color', this.op.secondColor).children(
					'a').css('color', this.op.activeFontColor)
		}
		;
		this.subMenu.css({
			'background-color' : this.op.mainColor,
	...
	Zmenu.prototype.createMobile = function(ele) {
		...
		this.allLinks
				.each(function(indx, element) {
					var hrf = $(element).attr('href'),target=$(element).attr('target'), txt = $(element).text(), hclass = $(
							element).parents('.zlight-submenu').length, tr = '';
					if (hclass > 0) {
						for (i = 0; i < hclass; i++) {
							tr += '-'
						}
					...
					;
					ele.mobileSelect.append('<option target="' + target + '" value="' + hrf + '">' + tr
							+ ' ' + txt + '</option>')
				})
	};
	Zmenu.prototype.change = function(ele) {
		this.mobileSelect.on('change.zmenuChange', function() {
			var target = $(this.options[this.selectedIndex]).attr("target");
			var url=this.options[this.selectedIndex].value;
			if(url.substring(0,11)=='javascript:'){
				var scriptStr=url.substring(11,url.length);
				eval(scriptStr);
				return;
			}
			if(target!=null&target!=''&typeof(target)!='undefined'&target!='undefined'){
				if(url!=null&url!=''){
		...
		this.li.add(this.subli).on(
				...
				function(event) {
					if (event.type === 'mouseenter') {
						$(this).css('background-color', ele.op.secondColor)
								.children('a').css('color',
										ele.op.activeFontColor)
