...
function doSearch(){
	...
	$.post("ticket/findByAttrBizTicketDepolyDetail.zb",formdata,function(data){
			if(data.success){
...
function saveData(){
	...
	$.post("ticket/saveBizTicketDepolyDetail.zb",formdata,function(data){
			if(data.success){
...
function deleteRows(){
	...
	if(rows.length<1){
		...
		var obj={};
        		...
        		var depolyDetailId=new Array();
				for(var i=0;i<rows.length;i++){
					depolyDetailId.push(rows[i].depolyDetailId);
				}
				obj.depolyDetailId=depolyDetailId;
		
		
		
		$.post("ticket/deleteBizTicketDepolyDetail.zb",obj,function(data){
			if(data.success){
...
<div id="dataTabelToolBar" style="height: auto">
			...
			<form id="queryForm">
					...
					:<input name="depolyDetailId" class="easyui-textbox" style="width:100px"> 
...
<div id="dataDialog" class="easyui-dialog" title="信息" style="width:400px;height:200px;padding: 15px"   
    ...
    <table>
    	<input type="hidden" name="depolyDetailId" value=""/>
