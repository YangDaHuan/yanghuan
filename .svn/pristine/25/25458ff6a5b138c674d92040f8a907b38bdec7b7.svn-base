...
function lianDong(newValue,oldValue) {
        ...
        if (newValue!=oldValue) {  
            $('#applyCode').combobox({  
                url: "ticket/findByAttrBizTicketApply.zb?auditingState=3&toOrg=" + newValue  
            });  
        }  
...
<form id="dataForm" >   
       ...
       <td align="right" >   
        <label for="applyCode">申请单号:</label>   
			...
			<input id="applyCode"  name="applyCode" class="easyui-combobox" 
								style="width: 150px;"
								data-options="
						    url: 'ticket/findByAttrBizTicketApply.zb?auditingState=3&toOrg=<%=sys_orgid %>',
							required:false,
							panelHeight:'auto', 
    ...
    <thead>   
        <tr>
      			...
      			<th data-options="field:'ticketTypeId',width:114,formatter:typeFormatter,editor:{type:'combobox',options:{data:getTicketTypes(),valueField:'ticketTypeId',textField:'ticketTypeName',editable:false,required:true}}" >票卡类型</th>  
      			<th data-options="field:'ticketStateId',width:112,formatter:stateFormatter,editor:{type:'combobox',options:{data:getTicketStates(),valueField:'ticketStateId',textField:'ticketStateName',editable:false,required:true}}" >票卡状态</th>  
      			<th data-options="field:'ticketMoney',width:112,editor: { type: 'numberbox', options: { min:0,max:1000000,required: false} }" >金额</th>  
        		<th data-options="field:'ticketNumber',width:112,editor: { type: 'numberbox', options: { min:0,max:1000000,required: true} }" >数量</th> 
