...
<#list model.fields as field>
	...
	for(var i=0; i<${field.no}Data.length; i++){
		...
		<#elseif field.dsType==3>			
			<#if field.type=='select'>
		$(".${field.no}-combo").combobox("reload", "df/ds.zb?fieldId=${field.id}");
			</#if>				
		</#if>
	</#if>
</#list>
...
<div class="searchDiv" region="north" title="${model.name}查询" split="true" style="height:72px; padding: 5px;">
	<form id="searchForm">
	<table width="100%">	
			<#assign queryIndex=0/>	
			<#assign queryBtn=0/>
			...
			<#list model.fields as field>				
				...
				<#if queryIndex==0||queryIndex%4==0>
					<#if (queryIndex>0)><#assign queryWidth=queryWidth+1/></#if>
		<tr height="25">
				</#if>
			<td>${field.name}:</td>
			<td>
	    		...
	    		<input class="easyui-combobox ${field.no}-combo" id="${field.no}" name="${field.no?upper_case}" data-options="
	                valueField:'ID',
	                textField:'TEXT',
	                panelHeight:'auto'">
	            <#else>
	            <input type="text" id="${field.no}" name="${field.no?upper_case}" class="easyui-validatebox">
				</#if>
			</td>
				<#assign queryIndex=queryIndex+1/>
				<#if ((queryIndex>0)&&queryIndex%4==0)||(field_index+1)==model.fields?size>
			<#if (queryIndex==4||(field_index+1)==model.fields?size)&&queryBtn==0>
			<td align="right" colspan="2">
				<a class="easyui-linkbutton" iconCls="icon-search" onclick="doSearch(1);">查 询</a>
				<a class="easyui-linkbutton" iconCls="icon-search" onclick="doSearch(0);">查询全部</a>				
			</td>		
				<#assign queryBtn=1/>
			</#if>
		</tr>		
				...
				<#if (field_index+1)==model.fields?size&&queryBtn==0>
				<td align="right" colspan="2">
					<a class="easyui-linkbutton" iconCls="icon-search" onclick="doSearch(1);">查 询</a>
					<a class="easyui-linkbutton" iconCls="icon-search" onclick="doSearch(0);">查询全部</a>			
					<#assign queryBtn=1/>	
...
<!-- Dialog -->
<div id="dfDialog" class="easyui-dialog" style="width:600px;height:400px;padding:10px 20px"
...
<script type="text/javascript">
	...
	function newData(){
	    ...
	    dfForm.form('clear');
	    <#list model.fields as field>
        	<#if field.defaultValue??&&field.defaultValue!=''&&field.type!='checkbox'>
        		<#if field.type=='select'>
	...
	//查询
	 function doSearch(index){
	 	...
	 	var sf = $("#searchForm").serializeObject();
	 	for(var o in sf){
	 		if(sf[o] instanceof Array){
	 			var array = "";
	 			for(var i=0; i<sf[o].length; i++){
	 				array+=sf[o][i]+",";
	 			}
	...
	$(".searchDiv").css("height",(72+28*${queryWidth})+"px");	
</script>
</body>
</html>