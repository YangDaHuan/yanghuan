...
function doSearch(){
	...
	$.post("ticket/findByAttrBizTicketApplyDetail.zb",formdata,function(data){
			if(data.success){
...
function saveData(){
	...
	$.post("ticket/saveBizTicketApplyDetail.zb",formdata,function(data){
			if(data.success){
...
function deleteRows(){
	...
	if(rows.length<1){
		...
		var obj={};
        		...
        		var applyDetailId=new Array();
				for(var i=0;i<rows.length;i++){
					applyDetailId.push(rows[i].applyDetailId);
				}
				obj.applyDetailId=applyDetailId;
		
		
		
		$.post("ticket/deleteBizTicketApplyDetail.zb",obj,function(data){
			if(data.success){
...
<div id="dataTabelToolBar" style="height: auto">
			...
			<form id="queryForm">
					...
					:<input name="applyDetailId" class="easyui-textbox" style="width:100px"> 
    ...
    <thead>   
        <tr>
      			...
      			<th data-options="field:'applyId',width:100" ></th>  
...
<div id="dataDialog" class="easyui-dialog" title="信息" style="width:400px;height:200px;padding: 15px"   
    ...
    <table>
    	<input type="hidden" name="applyDetailId" value=""/>
    	
				
    	<tr><td>   
        <label for="applyId">:</label>   
        </td><td>
			<input name="applyId" class="easyui-textbox" style="width:100px"> 
       </td></tr> 
