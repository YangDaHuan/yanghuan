/**
 * Licensed under the GPL licenses http://www.gnu.org/licenses/gpl.txt
 * 
...
(function(d) {
	function a(g) {
		var h = d.extend(true, {}, d.fn.combobox.defaults, d(g).combobox(
				"options"));
...
(function(f) {
	function c(j) {
		...
		}).combo("textbox").val(h.customAttr.headervalue).attr("prompt",
				h.customAttr.headervalue).focus(function() {
	...
	function e(h) {
		var j = f.extend(true, {}, f.fn.combo.defaults,
				f.data(h, "combo").options);
				...
				.keyup(
						function(k) {
							if (f(this).val().length != 0
									&& (f(this).val().length % g.minLength == 0)
									&& g.url) {
								f
										.ajax({
											type : "POST",
...
(function(c) {
	...
	function m(A) {
		var y = c.extend(true, {}, c.fn.datagrid.defaults, c(A).datagrid(
				"options"));
		...
		var x = [
				{
					...
					onclick : function(B, D, C, A) {
						c.fn.datagrid.rowContextMenu.defaultEvents.doDelete(B,
								D, C, A)
					}
				},
				"-",
				{
					...
					iconCls : "icon-reload",
					onclick : function(B, D, C, A) {
						c.fn.datagrid.rowContextMenu.defaultEvents.doReload(B,
								D, C, A)
					}
				},
				{
					...
					onclick : function(B, D, C, A) {
						c.fn.datagrid.rowContextMenu.defaultEvents
								.doReloadThisPage(B, D, C, A)
					}
				} ];
		return x
	}
	function e(B) {
		var y = c.extend(true, {}, c.fn.datagrid.defaults, c(B).datagrid(
				"options"));
	...
	function f(D) {
		var F = c.extend(true, {}, c.fn.datagrid.defaults, c(D).datagrid(
				"options"));
	...
	function q(C) {
		var y = c.extend(true, {}, c.fn.datagrid.defaults, c(C).datagrid(
				"options"));
		...
		var A = function(J) {
			...
			c('<a href="javascript:void(0)">确定</a>').css("margin-left", "0px")
					.linkbutton({
						iconCls : "icon-ok"
					}).click(
							function() {
								var K = c(J).datagrid("getRowIndex",
										c(J).datagrid("getEditingRow"));
								if (!y.customAttr.onConfirmEdit.call(J, K)) {
									return
								}
								c(J).datagrid("endEdit", K)
							}).appendTo(E);
			c('<a href="javascript:void(0)">取消</a>').css("margin-left", "6px")
					.linkbutton({
						iconCls : "icon-cancel"
					}).click(
							function() {
	...
	function u(z) {
		var y = c.extend(true, {}, c.fn.datagrid.defaults, c(z).datagrid(
				"options"));
		...
		var B = function(H, E) {
			...
			var D = function(L) {
				var K = [];
				var J = c.grep(c.merge(c(z).datagrid("getColumnFields", true),
						c(z).datagrid("getColumnFields")), function(N, M) {
		...
		var A = function() {
			if (y.customAttr.tooltip.target == "row") {
				...
				if (y.customAttr.tooltip.fields
						&& c.isArray(y.customAttr.tooltip.fields)) {
					...
					c.each(y.customAttr.tooltip.fields, function() {
						var G = this;
						x(c("td[field=" + G + "]", E),
								y.customAttr.tooltip.formatter)
					})
				}
			}
		};
	...
	function b(y) {
		var x = c.extend(true, {}, c.fn.datagrid.defaults, c(y).datagrid(
				"options"));
		if (!x.pagination) {
			return
		}
		c(y).datagrid("addEventListener", {
			name : "onLoadSuccess",
			handler : function(z) {
	...
	function r(z) {
		...
		var x = function(C) {
			...
			if (C.rows.length == 0) {
				var D = c(
						"div.datagrid-view2>div.datagrid-header>div.datagrid-header-inner>table",
						B)[0];
				var A = c(
	...
	function w(B, C, D, E) {
		...
		var x = null;
		var x = (D == 1 ? F.header1 : F.header2)
				.find(">table>tbody>tr.datagrid-header-row>td[field=" + C + "]");
		if (D > E) {
			(E == 1 ? F.header1 : F.header2).find(
					">table>tbody>tr.datagrid-header-row").append(x)
		} else {
			(E == 1 ? F.header1 : F.header2).find(
					">table>tbody>tr.datagrid-header-row")
					.children("td[field]").each(function() {
		...
		var y = (D == 1 ? F.body1 : F.body2).find(">table>tbody>tr>td[field="
				+ C + "]");
		if (D > E) {
			...
			c.each(y, function(H, I) {
				G.finder.getTr(B, H, "body", E).children("td[field]").each(
						function() {
							if (A(C, c(this).attr("field"))) {
								c(this).before(I);
								return false
	...
	c.extend(c.fn.datagrid.defaults.editors, {
		...
		datetimebox : {
			init : function(x, z) {
				var y = c('<input type="text" class="easyui-datetimebox">')
						.appendTo(x);
				z = z || {};
				z = c.extend({}, z, {
...
(function(b) {
	function a(d, c) {
		...
		return b("<style>" + c + "</style>").attr("id", d).attr("type",
	...
	b
			.extend({
				mask : function(f) {
					...
					this.unmask(d);
					if (d.target != "body"
							&& b(d.target).css("position") == "static") {
					...
					var g = b(
							...
							.html(d.loadMsg).appendTo(d.target);
					setTimeout(function() {
			...
			c.each(e, function() {
				...
				if (c.isPlainObject(j)) {
					...
					if (j == "-") {
						...
						c(g).removeClass("menu-item").addClass("menu-sep")
								.removeAttr("style").empty()
					}
				}
			})
		}
...
(function(f) {
	...
	function n(s) {
		var p = f.extend(true, {}, f.fn.tabs.defaults,
				f.data(s, "tabs").options);
	...
	function j(s, q, o) {
		...
		if (f.inArray(o, f.fn.tabs.defaults.customAttr.fixedtabs) == -1
				&& !f(s).tabs("getTab", o).panel("options").closable) {
			q.menu("setText", {
				target : p,
	...
	function a(p, o) {
		var q = [];
		o++;
		f(p).children("div.tabs-header").find("ul li:nth-child(" + o + ")")
				.each(function() {
					q.push(this)
				});
	...
	function e(r) {
		...
		for ( var o = 0; o < p.length; o++) {
			var q = p[o];
			if (q.panel("options").closable == undefined
					|| !q.panel("options").closable) {
				s.push(q)
			}
		}
		return s
	...
	function d(v, r, o, w, t) {
		var s = f("<iframe>").attr("height", "100%").attr("width", "100%")
	...
	$.fn.toolbar = function(options, param) {
		...
		return this.each(function() {
			...
			if (state) {
				...
				$.data(this, "toolbar", {
					options : $.extend({}, $.fn.toolbar.defaults, $.parser
...
(function(f) {
	...
	function t(v) {
		var u = v.idField || v.textField || v.iconField || v.childrenField
				|| v.attributesField || v.attributes || false;
		return u ? true : false
	}
	function g(E, z) {
		if (!t(E)) {
		...
		var u = E.idField || "id", w = E.textField || "text", v = E.iconField
				|| "iconCls", B = E.parentField || "pid", x = E.attributes
				|| [];
		var C = [], D = [];
	...
	function c(D, A) {
		...
		var u = D.idField || "id", x = D.textField || "text", w = D.iconField
				|| "iconCls", C = D.childrenField || "children", B = D.attributesField
				|| "attributes", y = D.attributes || [];
		var v = function(F) {
			if (!F.id && F[u]) {
				F.id = F[u]
			}
	...
	f.fn.tree.contextmenu.defaultEvents = {
		moveup : function(y, x, z) {
			var v = f
					.extend(true, {}, f.fn.tree.defaults, f(z).tree("options"));
			var w = l(z, x);
			if (w) {
...
(function(d) {
	...
	function m(r) {
		var n = d.extend(true, {}, d.fn.treegrid.defaults, d(r).treegrid(
				"options"));
		...
		d.each(r, function(v, x) {
			...
			q.push({
				...
				onclick : function(z, y, C) {
					...
					if (!A) {
						d.fn.datagrid.headerContextMenu.defaultEvents
								.doHideColumn(C, B, z)
					} else {
						d.fn.datagrid.headerContextMenu.defaultEvents
								.doShowColumn(C, B, z)
	...
	function k(q) {
		var o = d.extend(true, {}, d.fn.treegrid.defaults, d(q).treegrid(
				"options"));
	...
	function a(q) {
		var p = d.extend(true, {}, d.fn.treegrid.defaults, d(q).treegrid(
				"options"));
	...
	function b(s) {
		var o = d.extend(true, {}, d.fn.treegrid.defaults, d(s).treegrid(
				"options"));
		...
		var q = function(y) {
			...
			d('<a href="javascript:void(0)">确定</a>').css("margin-left", "0px")
					.linkbutton({
						iconCls : "icon-ok"
					}).click(function() {
						var z = o.idField;
			...
			d('<a href="javascript:void(0)">取消</a>').css("margin-left", "6px")
					.linkbutton({
						iconCls : "icon-cancel"
					}).click(function() {
						var z = o.idField;
	...
	function h(p) {
		var o = d.extend(true, {}, d.fn.treegrid.defaults, d(p).treegrid(
				"options"));
		...
		var r = function(y, u) {
			...
			var t = function(B) {
				var A = [];
				var z = d.grep(d.merge(d(p).treegrid("getColumnFields", true),
						d(p).treegrid("getColumnFields")), function(D, C) {
		...
		var q = function() {
			if (o.customAttr.tooltip.target == "row") {
				...
				if (o.customAttr.tooltip.fields
						&& d.isArray(o.customAttr.tooltip.fields)) {
					...
					d.each(o.customAttr.tooltip.fields, function() {
						var v = this;
						n(d("td[field=" + v + "]", u),
								o.customAttr.tooltip.formatter)
					})
				}
			}
		};
	...
	d.fn.treegrid.defaults.loadFilter = function(p, r) {
		...
		if (u && u.parentField) {
			...
			for ( var q = 0, s = p.rows.length; q < s; q++) {
				if (p.rows[q][t] && p.rows[q][t] != "0"
						&& p.rows[q][n] != p.rows[q][t]) {
